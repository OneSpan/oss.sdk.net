...
namespace SDK.Examples
{
    ...
    public class CustomSenderInfoExampleTest
    {
		...
		public void VerifyResult()
        {
            ...
            IDictionary<string, OneSpanSign.Sdk.Sender> senders = assertSenderWasAdded(100, example.SenderEmail);
        ...
        {
            int i = 0;
            IDictionary<string, Sender> senders = example.OssClient.AccountService.GetSenders(Direction.ASCENDING, new PageRequest(1, numberOfResults));
            while (!senders.ContainsKey(senderEmail))
            {
                Assert.AreEqual(senders.Count, numberOfResults);
                senders = example.OssClient.AccountService.GetSenders(Direction.ASCENDING, new PageRequest(i++ * numberOfResults, numberOfResults));
            }
            return senders;
        }
    }
