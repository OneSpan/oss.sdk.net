...
namespace SDK.Examples
{
    ...
    public class SenderManipulationExampleTest
    {
        ...
        {
            int i = 0;
            IDictionary<string, Sender> senders = example.OssClient.AccountService.GetSenders(Direction.ASCENDING, new PageRequest(1, 100));
            while (!senders.ContainsKey(senderEmail))
            {
                if (senders.Count == 100)
                {
                    senders = example.OssClient.AccountService.GetSenders(Direction.ASCENDING, new PageRequest(i++ * 100, 100));
                }
                else
                {
                    return true;
