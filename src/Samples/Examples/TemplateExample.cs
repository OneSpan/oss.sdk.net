...
namespace SDK.Examples
{
    public class TemplateExample : SDKSample
    {
        ...
        override public void Execute()
        {
			...
			templateId = ossClient.CreateTemplate(superDuperPackage);
            DocumentPackage template = ossClient.GetPackage(templateId);
            ...
            ossClient.TemplateService.Update(template);

			document.Description = "Updated description";
            ossClient.TemplateService.UpdateDocumentMetadata(template, document);

			ossClient.TemplateService.DeleteDocument(templateId, "doc1");

			Console.WriteLine("Template {0} updated", templateId);

            instantiatedTemplateId = ossClient.CreatePackageFromTemplate(templateId,
