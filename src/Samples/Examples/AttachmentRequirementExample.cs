...
namespace SDK.Examples
{
    public class AttachmentRequirementExample : SDKSample
    {
        ...
        public AttachmentRequirementExample()
        {
            ...
            this.attachmentInputStream2 = File.OpenRead(new FileInfo(Directory.GetCurrentDirectory() + "/SampleDocuments/document-with-fields.pdf").FullName);
            this.attachmentInputStream3 = File.OpenRead(new FileInfo(Directory.GetCurrentDirectory() + "/SampleDocuments/extract_document.pdf").FullName);
        }
        ...
        override public void Execute()
        {
            ...
            ossClient.UploadAttachment(packageId, signer1Att1.Id, ATTACHMENT_FILE_NAME1, attachment1ForSigner1FileContent, SIGNER1_ID);
            ossClient.UploadAttachment(packageId, signer2Att1.Id, ATTACHMENT_FILE_NAME2, 
                                       new StreamDocumentSource(attachmentInputStream2).Content(), SIGNER2_ID);
            ossClient.UploadAttachment(PackageId, signer2Att2.Id, ATTACHMENT_FILE_NAME3, 
                                       new StreamDocumentSource(attachmentInputStream3).Content(), SIGNER2_ID);

            // Sender rejects Signer1's uploaded attachment
            ossClient.AttachmentRequirementService.RejectAttachment(packageId, signer1, NAME1, REJECTION_COMMENT);
            retrievedPackageAfterRejection = ossClient.GetPackage(packageId);
            ...
            ossClient.AttachmentRequirementService.AcceptAttachment(packageId, signer1, NAME1);
            retrievedPackageAfterAccepting = ossClient.GetPackage(packageId);
            ...
            DownloadedFile downloadedAttachment = ossClient.AttachmentRequirementService.DownloadAttachmentFile(packageId, attachment1Id);
            ...
            DownloadedFile downloadedAllAttachmentsForPackage = ossClient.AttachmentRequirementService.DownloadAllAttachmentFilesForPackage(packageId);
            ...
            DownloadedFile downloadedAllAttachmentsForSigner1InPackage = ossClient.AttachmentRequirementService.DownloadAllAttachmentFilesForSignerInPackage(retrievedPackage, signer1);
            ...
            DownloadedFile downloadedAllAttachmentsForSigner2InPackage = ossClient.AttachmentRequirementService.DownloadAllAttachmentFilesForSignerInPackage(retrievedPackage, signer2);
